...
import { ATTESTATION_LABEL } from "@v26labs/delivery-harness-kernel";
...
async function npmShim(options: { readonly logPath: string; readonly installs: boolean }): Promise<string> {
  ...
  const script = [
    ...
    ...(options.installs
      ? [
          'mkdir -p "$PWD/node_modules/@v26labs"',
          'ln -sfn "$PWD/packages/kernel" "$PWD/node_modules/@v26labs/delivery-harness-kernel"',
        ]
      : ["exit 3"]),
    "exit 0",
  ].join("\n");
...
async function harnessRoot(options: { readonly installed: boolean }): Promise<string> {
  ...
  if (options.installed) {
    await mkdir(path.join(root, "node_modules", "@v26labs"), { recursive: true });
    await symlink(path.join(root, "packages", "kernel"), path.join(root, "node_modules", "@v26labs", "delivery-harness-kernel"));
  }
  return root;
}
