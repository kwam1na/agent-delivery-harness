 ...
 *
 * The flag question is asked the way `packages/action/action.yml` asks it —
 * of the executable itself, not of a version number — but it is asked
 ...
 * runtime already holds cannot flake, costs nothing, and is the same question.
 *
...
export function resolveHookRuntimeArgs(probes: HookRuntimeProbes): HookRuntimeResolution {
  ...
  if (probes.acceptsFlag(STRIP_TYPES_FLAG)) return { ok: true, args: [STRIP_TYPES_FLAG] };
  return {
    ok: false,
 ...
 * executable's own enumeration of the flags it accepts, so the probe reads an
 * answer rather than computing one, and a runtime that dropped or renamed the
 * flag simply does not list it.
 */
