...
- **One canonicalizer**: RFC 8785 (JCS) implemented in-kernel with RFC test vectors, used for `manifestDigest`, `recordId`, and `workspaceId` alike. Athena's locale-dependent `stableJson` is deliberately not ported. Alternative (`canonicalize` npm package) rejected: supply-chain surface on the most security-critical path.
