...
- F1 disposition: V26-2016 confirmed producer PR #67 merged at `0ff5ee68619eb5fd2c7c06def3b40625e756f9b6`, with both mandated lenses aligned and concurrent private-environment controls passing. Hosted jobs did not start because of billing; V26-2016 owns that external limitation in the shared log. V26-2014 remains the consolidated producer owner.
- Harness validation: the repaired full preflight passed 144 files / 3,211 tests. Round-one correctness aligned; testing planted surviving failures in workflow ancestry enforcement and structural-code coverage, plus smaller Git-error, unclassified npm-error and NOTICE-copy evidence gaps. The executor added coverage within the original tickets and started a fresh full gate. These are review findings under the existing work, not duplicate friction tickets; independent verification remains pending.
- Observation-call recovery: a capture request used `sha256` where the runtime requires `digest`, and a version-two emit initially omitted `--event-id`. Both refused without changing admission; the executor corrected the documented fields and retained the actual review bytes successfully. These were caller mistakes, not demonstrated product defects.
