...
The `@v26labs/delivery-harness-*` packages are not published yet. Until they
are, you consume them from a checkout of this repository (with `npm ci` run
inside it), pointed at by `$DELIVERY_HARNESS_CHECKOUT`. Two things need wiring:
a `delivery-harness` command on your `PATH`, and
`@v26labs/delivery-harness-kernel` resolvable from your repository root (your
`harness.config.ts` imports it).
...
mkdir -p .delivery-harness/bin node_modules/@v26labs
ln -sfn "$DELIVERY_HARNESS_CHECKOUT/packages/kernel" node_modules/@v26labs/delivery-harness-kernel
...
orchestrator (the MCP server in `@v26labs/delivery-harness-mcp` exposes exactly
